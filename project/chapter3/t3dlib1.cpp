...
if (bob->curr_frame < 0 | bob->curr_frame > sizeof(bob->images))
	return (0);
...
if (bob->curr_frame < 0 | bob->curr_frame > sizeof(bob->images))
	return (0);
if (bob->curr_frame < 0 | FAILED(dest->Blt(&dest_rect, bob->images[bob->curr_frame],
          &source_rect,(DDBLT_WAIT | DDBLT_KEYSRC),
          NULL)))
    return(0);
...
wchar_t* ConvertW(char* str, int sourceCodepage)
{
...
// draw the text a
wchar_t *wtext = ConvertW(text, CP_ACP);
//TextOut(xdc,x,y, (wchar_t*)text,strlen(text));
TextOut(xdc,x,y, wtext, wcslen(wtext));
